...
{
    printf("\n  [TC-AUD-03] setLevel – set+restore round-trip on SPEAKER0 (TV profile only)\n");

    std::string fname = mod_dir + "/setLevel_SPEAKER0_roundtrip.txt";
    TeeStream out;
    out.open(fname);
    ...
    /* ── Profile guard: only applicable on TV ──────────────────────────────────────────── */
    if (g_profile != PROFILE_TV && g_profile != PROFILE_ALL) {
        out << "Profile is not TV – TC-AUD-03 is a TV-only test (SPEAKER0).\n";
        out << "RESULT: SKIP (profile=" << (g_profile == PROFILE_STB ? "STB" : "Common") << ")\n";
        verify_record("TC-AUD-03", "setLevel SPEAKER0 round-trip", true,
                      "skip – profile is not TV");
        out << "Saved to: " << fname << "\n";
        printf("  [TC-AUD-03] SKIP – profile is not TV (SPEAKER0 not applicable).\n");
        return;
    }
    ...
    try {
        ...
        verify_record("TC-AUD-03", "setLevel(SPEAKER0,50.0) set+restore round-trip", pass, detail);
    }
    catch (const device::Exception &e) {
        out << "EXCEPTION (device::Exception): " << e.what() << "\n";
        out << "NOTE: SPEAKER0 may not exist on this platform, or handle is stale.\n";
        out << "RESULT: FAIL – stale handle or SPEAKER0 not present\n";
        verify_record("TC-AUD-03", "setLevel(SPEAKER0,50.0) set+restore round-trip", false, e.what());
    }
    catch (...) {
        out << "EXCEPTION (unknown)\n";
        out << "RESULT: FAIL\n";
        verify_record("TC-AUD-03", "setLevel(SPEAKER0,50.0) set+restore round-trip", false, "unknown exception");
    }
    out << "Saved to: " << fname << "\n";
}
...
{
    printf("\n  [TC-AUD-04] setGain – set+restore round-trip on SPEAKER0 (TV profile only)\n");

    std::string fname = mod_dir + "/setGain_SPEAKER0_roundtrip.txt";
    TeeStream out;
    out.open(fname);
    ...
    /* ── Profile guard: only applicable on TV ──────────────────────────────────────────── */
    if (g_profile != PROFILE_TV && g_profile != PROFILE_ALL) {
        out << "Profile is not TV – TC-AUD-04 is a TV-only test (SPEAKER0).\n";
        out << "RESULT: SKIP (profile=" << (g_profile == PROFILE_STB ? "STB" : "Common") << ")\n";
        verify_record("TC-AUD-04", "setGain SPEAKER0 round-trip", true,
                      "skip – profile is not TV");
        out << "Saved to: " << fname << "\n";
        printf("  [TC-AUD-04] SKIP – profile is not TV (SPEAKER0 not applicable).\n");
        return;
    }
    ...
    try {
        ...
        verify_record("TC-AUD-04", "setGain(SPEAKER0,-3.0dB) set+restore round-trip", pass, detail);
    }
    catch (const device::Exception &e) {
        out << "EXCEPTION (device::Exception): " << e.what() << "\n";
        out << "NOTE: SPEAKER0 may not exist on this platform, or handle is stale.\n";
        out << "RESULT: FAIL – stale handle or SPEAKER0 not present\n";
        verify_record("TC-AUD-04", "setGain(SPEAKER0,-3.0dB) set+restore round-trip", false, e.what());
    }
    catch (...) {
        out << "EXCEPTION (unknown)\n";
        out << "RESULT: FAIL\n";
        verify_record("TC-AUD-04", "setGain(SPEAKER0,-3.0dB) set+restore round-trip", false, "unknown exception");
    }
    out << "Saved to: " << fname << "\n";
}
...
static void print_audio_tc_menu(void)
{
    ...
    printf("║  [1] TC-AUD-01  getAudioOutputPorts (all ports)             ║\n");
    ...
    printf("╠══════════════════════════════════════════════════════════════╣\n");
    printf("║  [A] Run ALL audio TCs (TC-AUD-01 … TC-AUD-13)              ║\n");
...
static void handle_audio_tc_submenu(void)
{
    ...
    while (in_menu) {
        ...
        /* Helper lambda: bump run counter, clear results, create run dir,
         * write run_info.txt. Returns the new run directory path. */
        auto start_tc_run = [&](const char *tc_label) -> std::string {
            ++g_run_count;
            g_verify_results.clear();
            printf("\n════════════════════════════════════════════════════════════════\n");
        ...
        case 'A': case 'a':
        {
            std::string rd = start_tc_run("TC-AUD-01..13 (ALL)");
            g_jctl_pid = journalctl_start(rd);
            run_module_audio(rd);
            write_global_verify_summary(rd);
            journalctl_stop(g_jctl_pid, rd); g_jctl_pid = -1;
...
static void print_tc_module_menu(void)
{
    ...
    printf("║  [2] CompositeIn   TC-CIN-01 … TC-CIN-02             (soon) ║\n");
    printf("║  [3] Display       TC-DSP-01 … TC-DSP-02             (soon) ║\n");
    printf("║  [4] FPD           TC-FPD-01 … TC-FPD-06             (soon) ║\n");
    printf("║  [5] HDMIIn        TC-HIN-01 … TC-HIN-02             (soon) ║\n");
    printf("║  [6] Host          TC-HST-01 … TC-HST-02             (soon) ║\n");
    printf("║  [7] VideoDevice   TC-VD-01  … TC-VD-02              (soon) ║\n");
    printf("║  [8] VideoPort     TC-VP-01  … TC-VP-02              (soon) ║\n");
    printf("╠══════════════════════════════════════════════════════════════╣\n");
...
static void handle_tc_module_submenu(void)
{
    ...
    while (in_menu) {
        ...
        case '5': case '6': case '7': case '8':
            printf("\n  ⚠ TC submenu for this module is not yet implemented.\n");
            printf("  Use main menu [%c] to run all TCs for that module.\n", ch);
            break;
        case 'B': case 'b':
            in_menu = false;
            break;
