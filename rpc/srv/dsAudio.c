...
IARM_Result_t _dsSetApplicationAudioConfig(void *arg)
{
    ...
    if (func == 0) {
        ...
        if (dllib) {
            ...
            if (func) {
                INT_DEBUG("dsSetApplicationAudioConfig_t is defined and loaded\r\n");
            }
            else {
                IARM_BUS_Unlock(lock);
                dlclose(dllib);
                INT_ERROR("dsSetApplicationAudioConfig_t is not defined\r\n");
                return IARM_RESULT_INVALID_STATE;
            }
            dlclose(dllib);
        }
...
IARM_Result_t _dsGetApplicationAudioConfig(void *arg)
{
    ...
    if (func == 0) {
        ...
        if (dllib) {
            ...
            if (func) {
                INT_DEBUG("dsGetApplicationAudioConfig_t is defined and loaded\r\n");
            }
            else {
                IARM_BUS_Unlock(lock);
                dlclose(dllib);
                INT_ERROR("dsGetApplicationAudioConfig_t is not defined\r\n");
                return IARM_RESULT_INVALID_STATE;
            }
            dlclose(dllib);
        }
