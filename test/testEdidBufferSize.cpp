...
void test_edid_buffer_size() {
    ...
    // Test case 4: memcpy_s should use destination size, not *length
    {
        unsigned char edidBytes[MAX_EDID_BYTES_LEN] = {0};
        // memcpy_s should use 1024 (destination size), not *length (0)
        assert(sizeof(edidBytes) >= MAX_EDID_BYTES_LEN);
        printf("  ✓ memcpy_s should use destination size (1024), not *length\n");
    }
